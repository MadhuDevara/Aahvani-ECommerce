        ...
        <div className="bg-lux-ivory border-b border-lux-gold/12 px-4 py-10">
          <div className="max-w-5xl mx-auto">
            <nav aria-label="Breadcrumb" className="flex items-center gap-1.5 text-xs text-lux-ink/40 mb-4">
              ...
              <span className="text-lux-gold">My Wishlist</span>
            </nav>
            <h1 className="font-serif text-3xl md:text-4xl font-semibold text-lux-ink">My Wishlist</h1>
          </div>
        </div>
      ...
      <div className="bg-lux-ivory border-b border-lux-gold/12 px-4 py-10">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex items-end justify-between flex-wrap gap-3">
            <div>
              <h1 className="font-serif text-3xl md:text-4xl font-semibold text-lux-ink flex items-center gap-3">
                My Wishlist
                {items.length > 0 && (
                  <span className="text-[0.65rem] font-semibold px-2.5 py-1 bg-lux-gold/12 text-lux-gold rounded-full font-sans">
            ...
            {items.length > 0 && (
              <button
                ...
                className="text-[0.65rem] tracking-[0.15em] uppercase font-medium text-lux-ink/35 hover:text-red-500 transition-colors flex items-center gap-1.5"
              >
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {items.length === 0 ? (
          <div className="flex items-center justify-center min-h-[40vh]">
            <div className="text-center max-w-sm">
              <div className="w-20 h-20 mx-auto mb-6 bg-lux-gold/10 flex items-center justify-center">
                <Heart size={34} strokeWidth={1} className="text-lux-gold/50" />
              </div>
              <h2 className="font-serif text-2xl font-semibold text-lux-ink mb-2">Your wishlist is empty</h2>
              <GoldDivider />
              <p className="text-sm text-lux-ink/45 font-light leading-relaxed my-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {items.map((item) => {
              ...
              return (
                <div key={item.id} className="group bg-white border border-lux-ink/8 hover:shadow-[0_8px_32px_color-mix(in_srgb,var(--lux-gold)_10%,transparent)] hover:border-lux-gold/20 transition-all duration-300">
                  {/* Image */}
                  <div
                    role="presentation"
                  ...
                  >
                    <div className="pointer-events-none relative z-0 h-full w-full">
                      <div className="absolute inset-0 flex items-center justify-center opacity-[0.14]" aria-hidden="true">
                        <Gem size={72} strokeWidth={0.8} className="text-lux-gold" />
                      </div>
                      <div className="absolute inset-0 bg-lux-gold/0 group-hover:bg-lux-gold/4 transition-colors duration-300" />
                      {discount > 0 && (
                    ...
                    >
                      <Heart size={14} strokeWidth={1.5} className="fill-lux-gold text-lux-gold" />
                    </button>
                  </div>

                  {/* Info */}
                  <div className="p-5">
                    <p className="text-[0.62rem] tracking-[0.15em] uppercase text-lux-ink/35 font-medium mb-1">
                      {item.category}
                    </p>
                    <Link href={`/shop/${encodeURIComponent(item.id)}`}>
                      <h3 className="font-serif text-base font-medium text-lux-ink mb-3 group-hover:text-lux-gold transition-colors leading-snug">
                        {item.name}
                      </h3>
                    </Link>
                    <div className="flex items-baseline gap-2 mb-4">
                      <span className="text-lux-gold font-semibold text-sm">{inr(item.price)}</span>
                      {item.originalPrice > item.price && (
                        <span className="text-lux-ink/30 text-xs line-through">{inr(item.originalPrice)}</span>
                      )}
                    </div>
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="w-10 flex items-center justify-center border border-lux-ink/12 text-lux-ink/30 hover:border-red-200 hover:text-red-500 hover:bg-red-50 transition-all duration-150"
                        aria-label="Remove"
                      >
