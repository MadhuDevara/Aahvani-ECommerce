...
function SectionCard({ title, sub, children }: { title: string; sub: string; children: React.ReactNode }) {
  return (
    <div className="bg-white border border-lux-ink/8 overflow-hidden">
      <div className="px-6 py-5 border-b border-lux-ink/6 bg-lux-ivory">
        <h2 className="font-serif text-lg font-semibold text-lux-ink">{title}</h2>
        <p className="text-[0.65rem] text-lux-ink/40 font-light mt-0.5">{sub}</p>
      </div>
...
function FieldInput(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      {...props}
      className={`w-full px-4 py-2.5 bg-lux-ivory border border-lux-ink/12 text-sm text-lux-ink placeholder-lux-ink/20 focus:outline-none focus:border-lux-gold/50 transition-colors disabled:opacity-40 disabled:cursor-not-allowed ${props.className ?? ''}`}
    />
  )
}
...
function Toggle({ on, onChange, id }: { on: boolean; onChange: () => void; id: string }) {
  return (
    <button
      ...
      aria-checked={on}
      onClick={onChange}
      className={`relative w-11 h-6 rounded-full transition-colors duration-200 flex-shrink-0 focus:outline-none ${on ? 'bg-lux-gold' : 'bg-lux-ink/15'}`}
    >
    ...
    <div className="bg-lux-ivory">
      ...
      <div className="bg-lux-ivory border-b border-lux-gold/12 px-4 py-10">
        <div className="max-w-3xl mx-auto">
          <nav aria-label="Breadcrumb" className="flex items-center gap-1.5 text-xs text-lux-ink/40 mb-5">
            <Link href="/" className="hover:text-lux-gold transition-colors">Home</Link>
            <ChevronRight size={11} strokeWidth={1.5} />
            <span className="text-lux-gold">My Profile</span>
          </nav>
          <div className="flex items-center gap-5">
            <div className="w-16 h-16 rounded-full bg-lux-gold flex items-center justify-center flex-shrink-0 shadow-md">
              <span className="font-serif text-2xl font-bold text-white">{initial}</span>
            </div>
            <div>
              <h1 className="font-serif text-3xl font-semibold text-lux-ink">My Profile</h1>
              <p className="text-sm text-lux-ink/40 font-light mt-0.5">{user?.email}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-7">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-[0.58rem] text-lux-ink/30 mt-1 font-light">Email cannot be changed here</p>
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-lux-ink/40 font-medium">+91</span>
                  <FieldInput
                    value={phone}
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              <button
                onClick={saveInfo}
                disabled={infoSaving}
                className="ml-auto flex items-center gap-2 px-7 py-2.5 bg-lux-gold text-white text-[0.68rem] tracking-[0.18em] uppercase font-medium hover:bg-lux-gold-hover transition-colors disabled:opacity-60"
              >
                {infoSaving
        ...
        <SectionCard title="My Addresses" sub="Manage your delivery addresses">
          <div className="space-y-4">
            {addresses.map((addr) => (
              <div key={addr.id} className="border border-lux-ink/8 p-4 relative hover:border-lux-gold/20 transition-colors">
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  <div className="flex items-start gap-3">
                    <div className="w-8 h-8 bg-lux-gold/10 flex items-center justify-center flex-shrink-0 mt-0.5">
                      <MapPin size={14} strokeWidth={1.5} className="text-lux-gold" />
                    </div>
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <p className="text-sm font-semibold text-lux-ink">{addr.label}</p>
                        {addr.isDefault && (
                          <span className="text-[0.55rem] font-bold tracking-wide px-2 py-0.5 bg-lux-gold/12 text-lux-gold rounded-full border border-lux-gold/20">
                            Default
                          </span>
                        )}
                      </div>
                      <p className="text-xs text-lux-ink/55 font-light leading-relaxed">
                        {addr.line1}{addr.line2 ? `, ${addr.line2}` : ''}<br />
                        {addr.city}, {addr.state} – {addr.pincode}
                      </p>
                      {!addr.isDefault && (
                        <button onClick={() => setDefault(addr.id)} className="text-[0.6rem] text-lux-gold hover:text-lux-gold-hover font-medium mt-1.5 tracking-wide transition-colors">
                          Set as default
                        </button>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    <button onClick={() => startEdit(addr)} className="w-8 h-8 flex items-center justify-center text-lux-ink/30 hover:text-lux-gold border border-transparent hover:border-lux-gold/20 transition-all" aria-label="Edit address">
                      <Pencil size={13} strokeWidth={1.5} />
                    </button>
                    <button onClick={() => deleteAddress(addr.id)} className="w-8 h-8 flex items-center justify-center text-lux-ink/30 hover:text-red-500 border border-transparent hover:border-red-100 transition-all" aria-label="Delete address">
            ...
            {/* Add/edit form */}
            {showAddForm ? (
              <div className="border border-lux-gold/20 bg-lux-ivory/60 p-5 space-y-4">
                <p className="text-[0.65rem] tracking-[0.2em] uppercase font-semibold text-lux-gold">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <select value={newAddr.state} onChange={(e) => setNewAddr((a) => ({ ...a, state: e.target.value }))} aria-label="State" className="w-full px-4 py-2.5 bg-lux-ivory border border-lux-ink/12 text-sm text-lux-ink focus:outline-none focus:border-lux-gold/50 transition-colors">
                ...
                <label className="flex items-center gap-2.5 cursor-pointer">
                  <input type="checkbox" checked={newAddr.isDefault} onChange={(e) => setNewAddr((a) => ({ ...a, isDefault: e.target.checked }))} className="w-4 h-4 border border-lux-gold accent-lux-gold" />
                  <span className="text-sm text-lux-ink/65 font-light">Set as default address</span>
                </label>
                <div className="flex gap-3">
                  <button onClick={saveAddress} className="px-6 py-2.5 bg-lux-gold text-white text-[0.68rem] tracking-[0.18em] uppercase font-medium hover:bg-lux-gold-hover transition-colors">
                    {editingAddress ? 'Update Address' : 'Save Address'}
                  </button>
                  <button onClick={() => { setShowAddForm(false); setEditingAddress(null); setNewAddr({ label: '', line1: '', line2: '', city: '', state: '', pincode: '', isDefault: false }) }} className="px-6 py-2.5 border border-lux-ink/15 text-lux-ink/50 text-[0.68rem] tracking-[0.18em] uppercase font-medium hover:border-lux-ink/25 transition-colors">
                    Cancel
                  </button>
                </div>
              </div>
              ...
              <button
                ...
                className="flex items-center gap-2 text-[0.68rem] tracking-[0.15em] uppercase font-medium text-lux-gold hover:text-lux-gold-hover transition-colors border border-dashed border-lux-gold/30 hover:border-lux-gold/50 w-full py-3 justify-center"
              >
          ...
          <div className="space-y-7">
            ...
            <div>
              <div className="flex items-center gap-2 mb-4">
                <Lock size={14} strokeWidth={1.5} className="text-lux-gold" />
                <h3 className="text-sm font-semibold text-lux-ink">Change Password</h3>
              </div>
              <div className="space-y-3 max-w-sm">
                    ...
                    <div key={key}>
                      ...
                      <div className="relative">
                        ...
                        <button
                          ...
                          className="absolute right-3 top-1/2 -translate-y-1/2 text-lux-ink/25 hover:text-lux-ink/50 transition-colors"
                ...
                <button
                  ...
                  disabled={pwSaving}
                  className="px-6 py-2.5 bg-lux-gold text-white text-[0.68rem] tracking-[0.18em] uppercase font-medium hover:bg-lux-gold-hover transition-colors disabled:opacity-60 flex items-center gap-2"
                >
            ...
            <div>
              <div className="flex items-center gap-2 mb-4">
                <Bell size={14} strokeWidth={1.5} className="text-lux-gold" />
                <h3 className="text-sm font-semibold text-lux-ink">Email Preferences</h3>
              </div>
              <div className="space-y-1">
                {[
                  ...
                  <div key={key} className="flex items-center justify-between py-3 border-b border-lux-ink/5 last:border-0">
                    <div>
                      <label htmlFor={`notif-${key}`} className="text-sm font-medium text-lux-ink cursor-pointer">{label}</label>
                      <p className="text-[0.62rem] text-lux-ink/40 font-light mt-0.5">{sub}</p>
                    </div>
